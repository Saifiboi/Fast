...
public:
	...
	// string recommend(string word,int &Flag){
	// 	string ret_str;
	// 	if(searchWord(word)){
	// 		Flag=0;
	// 		return ret_str;
	...
	// 	int j=0;
	// 	cout<<ln.size();
	// 	for(vector< pair<int, string> > :: iterator itr = ln.begin(); itr!= ln.end(); itr++,j++){
	// 		cout<<"\nhere:";
	// 		if(j==0)
	// 		{
	// 			ret_str=itr->second;	
	...
	string recommend(string word){
		...
		cout<<ln.size();
		int i = 1;
		for(vector< pair<int, string> > :: iterator itr = ln.begin(); itr!= ln.end() && i<=4; itr++){
...
string BKTree::search(string w, int t)
{
...
string BKTreeSearch(string line){
	string word = line;
	...
	// // start = clock();
	// cout<<"Searching";
	string suggestions=tree->search(word, 1);
	// end = clock();
