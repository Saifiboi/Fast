#include <string>
#include <iostream>
#include "enc.h"
using namespace std;



    void Subs_bytes_Dec(unsigned char State_array[][4]){            //Function for substituting bytes from S inverse-box.

        unsigned char sboxin[17][17] =
        {
        };
        char row_check;
        char col_check;
        for(int l=0;l<4;l++){
            for(int i = 0;i < 4 ; i++){
                row_check = State_array[l][i] >> 4;
                col_check = State_array[l][i] ^ (row_check<<4);
                for(int j=1;j<17;j++){
                    if(sboxin[j][0] == row_check){
                        for(int k=1;k<17;k++){
                            if(sboxin[0][k]==col_check){
                                State_array[l][i] = sboxin[j][k];
                            }
                        }
                    }
                }
            }
        }
    }


    void shift_call_Dec(unsigned char State_array[][4]){        //Function for sceduling shifters for State Array in anti Direction.
        Shift_rows_S(State_array[1],1);
        Shift_rows_S(State_array[2],1);
        Shift_rows_S(State_array[2],1);
        Shift_rows_S(State_array[3]);
    }

    void Mix_col_inverse(unsigned char State_array[][4]){      //Function for inverse mixing columns of State Array.
        unsigned char const_inv_matrix[4][4] = {
           0x0E , 0x0B , 0x0D , 0x09
         , 0x09 , 0x0E , 0x0B , 0x0D
         , 0x0D , 0x09 , 0x0E , 0x0B
         , 0x0B , 0x0D , 0x09 , 0x0E
          };
           unsigned char column[4];
          for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                column[j] = State_array[j][i];
            }
            multiply_bridge(column,const_inv_matrix);
            for(int j=0;j<4;j++){
                State_array[j][i] = column[j];
            }

          }
    }

    void decrypt(unsigned char State_Array[][4],unsigned char** key){       //Function for sceduling  one Decryption round.
        shift_call_Dec(State_Array);
        Subs_bytes_Dec(State_Array);
        key_add(key,State_Array);
        Mix_col_inverse(State_Array);        
    }









