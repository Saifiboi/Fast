...
char Patient::patientmenu(Doctor *D)
{
	...
	for(int i=0;i<5;i++)
	{
		if(this->calllog[i]==-1)
		{
			...
			this->C[i].readcall("/home/safi/Documents/Semester2/OOP/Project/Cloud/Patient"+to_string(this->patient_id)+"/CALL/CALL"+to_string(this->D[i]->getDoctorId())+".dat");
			cout<<"\n He says:"<<this->C[i].getdocres()<<endl;
			this->C[i].takecallbyP();
			this->C[i].savecall("/home/safi/Documents/Semester2/OOP/Project/Cloud/Patient"+to_string(this->patient_id)+"/CALL/CALL"+to_string(this->D[i]->getDoctorId())+".dat");
			this->normalcallin(i);
		}
	}
	...
	case 6:
		...
		if(this->D[app_choice-1]->getcallin(this->patient_id-1)==-1)
		{
