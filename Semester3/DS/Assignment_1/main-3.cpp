...
//         string prev_hash, date, time, action;
//         User user;

//         char prev_hash_buf[256]; // Adjust the size as needed
//         char date_buf[256];      // Adjust the size as needed
//         char time_buf[256];      // Adjust the size as needed
//         char action_buf[256];    // Adjust the size as needed

//         // Read individual data fields
//         file.read(prev_hash_buf, sizeof(prev_hash_buf));
//         file.read(date_buf, sizeof(date_buf));
//         file.read(time_buf, sizeof(time_buf));
//         file.read(action_buf, sizeof(action_buf));
//         file.read(reinterpret_cast<char*>(&user), sizeof(User));

//         // Null-terminate the char arrays
//         prev_hash_buf[sizeof(prev_hash_buf) - 1] = '\0';
//         date_buf[sizeof(date_buf) - 1] = '\0';
//         time_buf[sizeof(time_buf) - 1] = '\0';
//         action_buf[sizeof(action_buf) - 1] = '\0';

//         // Assign the data fields to the new node
//         prev_hash = prev_hash_buf;
//         date = date_buf;
//         time = time_buf;
//         action = action_buf;

//         // Check the hash to ensure data integrity (compare with node_hash)
//         string calculated_hash = hasher(prev_hash + date + time + action + user);
//         if (calculated_hash != node_hash) {
//             cerr << "Data corruption detected! Skipping this node." << endl;
//             continue;
//         }

//         // Construct a new node and insert it into the chain
//         hash_node<User> node(prev_hash, date, time, action, user);
//         this->insert_end2(prev_hash, date, time, action, user);
//     }
...
// }
    ...
    hash_node<User>* current = head;
    while (current != nullptr) {
        // Serialize the node data to a stringstream
        stringstream ss;
        ss << current->prev_hash << " " << current->date << " " << current->time << " " << current->action << " ";
        ss << current->user.serialize(); // Assuming User has a serialize method

        // Get the serialized data as a string
        string serialized_data = ss.str();
        size_t data_size = serialized_data.size();

        // Write the size of the data and the data itself to the file
        file.write(reinterpret_cast<const char*>(&data_size), sizeof(data_size));
        file.write(serialized_data.c_str(), data_size);
...
void readHashChainFromFile(const string& filename) {
    ...
    while (true) {
        ...
        file.read(&serialized_data[0], data_size);

        // Deserialize the data to construct a new node
        stringstream ss(serialized_data);
        hash_node<User> node("", "", "", "", User()); // Initialize a default node

        ss >> node.prev_hash >> node.date >> node.time >> node.action;
        node.user.deserialize(ss); // Assuming User has a deserialize method
